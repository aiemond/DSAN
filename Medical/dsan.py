...
class MVIDataset(Dataset):
    ...
    def process_cat_vars(self, x):
        cat_tensors = [torch.tensor(int(v), dtype=torch.long) if not np.isnan(v) else torch.tensor(0)
...
class Imputer(object):
    ...
    # Check if rec_cat_vars is empty and handle accordingly
        if rec_cat_vars:
            rec_cat_vars = np.concatenate(rec_cat_vars, axis=1)
        else:
        # Initialize rec_cat_vars as an empty array with shape (batch_size, 0) if there are no categorical variables
            rec_cat_vars = np.empty((batch_size, 0), dtype='object')

        result = np.zeros((batch_size, self.n_col))
        result[:, self.num_vars] = rec_num_vars
        result = result.astype('object')
        result[:, self.cat_vars] = rec_cat_vars  # Make sure self.cat_vars is correctly set to handle empty rec_cat_vars
        return result
